...
";
            else if (rb.Equals(rbMsgCsiSample3))
            {
                ...
                txtMsgBoxText.Text = $"註冊完畢。\n\n歡迎使用「台中捷運綠線-列車地面管理程式 TGLGround」。\n\n您的使用期限為：\x1B[32m無期限\x1B[39m";
            }
        ...
        private void btnAttach_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog ofd = new OpenFileDialog())
            {
                ...
                if (string.IsNullOrEmpty(txtFile.Text))
                    ...
                    ofd.FileName = Path.GetFileName(txtFile.Text.Split(';')[0]);
                DialogResult dr = ofd.ShowDialog(this);
                if (dr != DialogResult.OK)
                    return;
                txtAttachments.Text = string.Join(";", ofd.FileNames);
            }
        }
